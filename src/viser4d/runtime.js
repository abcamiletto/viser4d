...
(() => {
  ...
  var TimelineRuntime = class {
    ...
    syncTimestepToServer(step, force = false) {
      ...
      if (!force && this.playing && this.config.fps > this.maxTimestepSyncHz && now - this.lastSyncSentAt < 1e3 / this.maxTimestepSyncHz) {
        return;
      }
    ...
    resetTimelineState(targetStep = 0) {
      ...
      getWindow().requestAnimationFrame(() => {
        ...
        getWindow().requestAnimationFrame(() => {
          ...
          const targetStep2 = this.resetTargetStep ?? 0;
          this.resetTargetStep = null;
          if (targetStep2 >= 0) {
            this.applyThrough(targetStep2);
          }
        });
      });
    }
