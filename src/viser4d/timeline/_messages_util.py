...
def scene_entries_for_message(
    ...
    """Convert one raw scene message into canonical keyed puts/deletes."""
    message_type = message.payload.get("type")
...
def scene_message_state_key(message: StoredMessage) -> str | None:
    ...
    if isinstance(name_field, str):
        prefix = _scene_node_prefix(name_field)
    else:
        prefix = "scene.global"
...
def step_patch_payload(step: TimelineStep) -> StoredStatePatch:
    """Convert one in-memory step patch into a transport payload."""
    return {
        "scenePuts": [
            {
                "key": key,
...
def timeline_step_from_patch_payload(payload: StoredStatePatch) -> TimelineStep:
    return TimelineStep(
        scene_puts={
            entry["key"]: entry["message"]
            for entry in payload["scenePuts"]
        },
        scene_delete_nodes=list(payload["sceneDeleteNodes"]),
        audio_messages=list(payload["audioMessages"]),
    )


def scene_delete_state_key(name: str) -> str:
    return f"{_scene_node_prefix(name)}:delete"


def _scene_node_prefix(name: str) -> str:
    return "scene.root" if name == "" else f"scene.node:{name}"
